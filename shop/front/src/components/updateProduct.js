...
function UpdateProduct({product, back}) {
    ...
    function changeCharecteristics(event) {
        setCharecteristics(event.target.value)
    }
    ...
    useEffect(() => {
        ...
        setCharecteristics(product.charecteristics)
        setImages(product.images)
        setPrice(product.price)
        setCategory(product.category)
        setNumber(product.number)
        setAbout(product.about)
        const categoriesAutoFill = JSON.parse(product.charecteristics)
        if(categoriesAutoFill.ageCategory) {
            setChAgeCategory(categoriesAutoFill.ageCategory)
        }
        if(categoriesAutoFill.alcoholContent) {
            setChAlcoholContent(categoriesAutoFill.alcoholContent)
        }
        if(categoriesAutoFill.boxType) {
            setChBoxType(categoriesAutoFill.boxType)
        }
        if(categoriesAutoFill.capacity) {
            setChCapacity(categoriesAutoFill.capacity)
        }
        if(categoriesAutoFill.brand) {
            setChBrand(categoriesAutoFill.brand)
        }
        if(categoriesAutoFill.color) {
            setChColor(categoriesAutoFill.color)
        }
        if(categoriesAutoFill.count) {
            setChCount(categoriesAutoFill.count)
        }
        if(categoriesAutoFill.type) {
            setChType(categoriesAutoFill.type)
        }
        if(categoriesAutoFill.useType) {
            setChUseType(categoriesAutoFill.useType)
        }
        if(categoriesAutoFill.scent) {
            setChScent(categoriesAutoFill.scent)
        }
        if(categoriesAutoFill.gender) {
            setChGender(categoriesAutoFill.gender)
        }
        if(categoriesAutoFill.country) {
            setChCountry(categoriesAutoFill.country)
        }
        if(categoriesAutoFill.purpose) {
            setChPurpose(categoriesAutoFill.purpose)
        }
        if(categoriesAutoFill.mass) {
            setChMass(categoriesAutoFill.mass)
        }
        if(categoriesAutoFill.texture) {
            setChTexture(categoriesAutoFill.texture)
        }
        if(categoriesAutoFill.effect) {
            setChEffect(categoriesAutoFill.effect)
        }
        if(categoriesAutoFill.coverage) {
            setChCoverage(categoriesAutoFill.coverage)
        }
        if(categoriesAutoFill.protectionDegree) {
            setChProtectionDegree(categoriesAutoFill.protectionDegree)
        }
        if(categoriesAutoFill.skinType) {
            setChSkinType(categoriesAutoFill.skinType)
        }
        if(categoriesAutoFill.material) {
            setChMaterial(categoriesAutoFill.material)
        }
        if(categoriesAutoFill.hairType) {
            setChHairType(categoriesAutoFill.hairType)
        }
        if(categoriesAutoFill.fixationDegree) {
            setChFixationDegree(categoriesAutoFill.fixationDegree)
        }
        if(categoriesAutoFill.time) {
            setChTime(categoriesAutoFill.time)
        }
        if(categoriesAutoFill.format) {
            setChFormat(categoriesAutoFill.format)
        }
        if(categoriesAutoFill.usage) {
            setChUsage(categoriesAutoFill.usage)
        }
        if(categoriesAutoFill.style) {
            setChStyle(categoriesAutoFill.style)
        }
    }, [])
        ...
        if (!name || !shopName || !shopID || !images || !price || !category || !number || !about) {
            ...
            if (category === 'Liquid soap' || category === 'Shower gels for bath') {
                if (chType.length === 0) {
                    ...
                    notification.addNotification({
                        type: 'warning',
                        name: 'Характеристика обєм не може бути порожньою'
                    })
                } else if (chPurpose.length === 0) {
                    notification.addNotification({
                        type: 'warning',
            ...
            if(char) {
                await axios({
                    method: "put",
                    url: "http://localhost:3000/product/update/" + product.id,
                    data: stringify({
                        name: name,
                        shopName: shopName,
                        shopID: shopID,
                    ...
                    if (response.data !== '') {
                        ...
                        setCharecteristics('')
                        back()
                        notification.addNotification({type: 'success', name: 'Ви відреагували товар'})
                    }
        ...
        <div className={'addUser'}>
            ...
            <div className={'addUserInputsHolder'}>
                <div className={'addProductsInputs'}>
                    ...
                    <div className='logRegInput'>
                        <input type="text" value={images} onChange={changeImages} placeholder='Фото(більше 1 вставте між ними (;))'
                    ...
                    <div className='logRegInput'>
                        ...
                        <datalist id="cosmeticCategory">
                            ...
                            <option value="Shower gels for bath, Гелі для дшу(догляд)">Гелі для дшу(догляд)</option>
                    ...
                    <div className='logRegInput'>
                        <input type="text" value={number} onChange={changeNumber} placeholder='Ціна'
                               className={number.length > 0 ? 'green' : 'red'}/>
                    </div>

                    <div className='logRegInput'>
                        <input type="text" value={about} onChange={changeAbout} placeholder='Напишіть щось про товар'
                                    ...
                                    <>
                                        ...
                                        <div className='logRegInput'>
                                            <input type="text" value={chProtectionDegree}
                                                   onChange={changeChProtectionDegree} placeholder='Ступінь захисту'
                                ...
                                <>
                                    ...
                                    <div className='logRegInput'>
                                        <input type="text" value={chHairType} onChange={changeChHairType}
                                               placeholder='Тип шкіри/проблема'
                            ...
                            category === 'Makeup accessories' || category === 'Cosmetic bags' ?
                                ...
                                category === 'Gift for her' || category === 'Gift for him' ?
                                    <>
                                        ...
                                        <div className='logRegInput'>
                                            <input type="text" value={chUseType} onChange={changeChUseType}
                                                   placeholder='Спосіб використання'
                                                   className={chUseType.length > 0 ? 'green' : 'red'}/>
                                        </div>
            ...
            <div className={'addUserButtonHolder'}>
                <div className={'addUserButton'} onClick={updateProduct}>
                    Змінити товар
                </div>
            </div>
        </div>
